...
public class CharacterController2D : MonoBehaviour
{
    ...
    private bool leapMotionleft;
    private bool leapMotionRight;
 	private bool leapMotionJumpTop;
 	private bool leapMotionBott;
 	private bool leapMotionAttackAnimation1;
 	private bool leapMotionAttackAnimation2;
    ...
    void Start()
    {
		...
		leapMotionleft  = false;
    	leapMotionRight= false;
 		leapMotionJumpTop= false;
 		leapMotionBott= false;
 		leapMotionAttackAnimation1= false;
 		leapMotionAttackAnimation2= false;
	...
	void Update()
    {
        ...
        //leap Motion Integrar 
        if (keyboard.leftArrowKey.isPressed || keyboard.aKey.isPressed || leapMotionleft)
            moveHorizontal = -1.0f;
        else if (keyboard.rightArrowKey.isPressed || keyboard.dKey.isPressed || leapMotionRight)
            moveHorizontal = 1.0f;

        movementInput = new Vector2(moveHorizontal, 0);

        // Jumping input
        if (!isJumping && (keyboard.spaceKey.wasPressedThisFrame || leapMotionJumpTop ))
            jumpInput = true;
    }
    //Trigger Movimiento Left Active
    public void moveLeft(){
		leapMotionleft  = true;
    }
    //Trigger Movimiento Left Desactive
    public void moveLeftDesactive(){
		leapMotionleft  = false;
    }
    //Trigger Movimiento Right Active
    public void moveRight(){
    	leapMotionRight= true;  	
    }
    //Trigger Movimiento Right Desactive
    public void moveRightDesactive(){
        leapMotionRight= false;     
    }
    //Trigger Movimiento Jump Active
    public void moveJump(){
 		leapMotionJumpTop= true;
    }
    //Trigger Movimiento Jump Desactive
    public void moveJumpDesactive(){
        leapMotionJumpTop = false;
    }
    void FixedUpdate()
    {
